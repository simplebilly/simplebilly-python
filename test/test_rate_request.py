# coding: utf-8

"""
    SimpleBilly API

    Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

    The version of the OpenAPI document: 0.1.0
    Contact: support@simplebilly.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from simplebilly_api.models.rate_request import RateRequest

class TestRateRequest(unittest.TestCase):
    """RateRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> RateRequest:
        """Test RateRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `RateRequest`
        """
        model = RateRequest()
        if include_optional:
            return RateRequest(
                customer = simplebilly_api.models.customer_info.CustomerInfo(
                    annual_volume = 0, 
                    is_registered = True, ),
                packages = [
                    simplebilly_api.models.package.Package(
                        description = '', 
                        height_cm = 1.337, 
                        length_cm = 1.337, 
                        reference = '', 
                        weight_kg = 1.337, 
                        width_cm = 1.337, )
                    ],
                recipient = simplebilly_api.models.address.Address(
                    city = '', 
                    company = '', 
                    country = '', 
                    email = '', 
                    name = '', 
                    phone = '', 
                    street = '', 
                    street_number = '', 
                    zip = '', ),
                sender = simplebilly_api.models.address.Address(
                    city = '', 
                    company = '', 
                    country = '', 
                    email = '', 
                    name = '', 
                    phone = '', 
                    street = '', 
                    street_number = '', 
                    zip = '', )
            )
        else:
            return RateRequest(
                packages = [
                    simplebilly_api.models.package.Package(
                        description = '', 
                        height_cm = 1.337, 
                        length_cm = 1.337, 
                        reference = '', 
                        weight_kg = 1.337, 
                        width_cm = 1.337, )
                    ],
                recipient = simplebilly_api.models.address.Address(
                    city = '', 
                    company = '', 
                    country = '', 
                    email = '', 
                    name = '', 
                    phone = '', 
                    street = '', 
                    street_number = '', 
                    zip = '', ),
                sender = simplebilly_api.models.address.Address(
                    city = '', 
                    company = '', 
                    country = '', 
                    email = '', 
                    name = '', 
                    phone = '', 
                    street = '', 
                    street_number = '', 
                    zip = '', ),
        )
        """

    def testRateRequest(self):
        """Test RateRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
