# coding: utf-8

"""
    SimpleBilly API

    Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

    The version of the OpenAPI document: 0.1.0
    Contact: support@simplebilly.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from simplebilly_api.models.posting_category_update import PostingCategoryUpdate

class TestPostingCategoryUpdate(unittest.TestCase):
    """PostingCategoryUpdate unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PostingCategoryUpdate:
        """Test PostingCategoryUpdate
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PostingCategoryUpdate`
        """
        model = PostingCategoryUpdate()
        if include_optional:
            return PostingCategoryUpdate(
                account_number = '',
                account_number_skr03 = '',
                account_number_skr04 = '',
                account_number_skr49 = '',
                category_type = 'income',
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                default_vat_rate = 56,
                description = '',
                eks_category = '',
                eu_vat_line = 56,
                input_vat_percentage = '',
                is_active = True,
                is_system = True,
                name = '',
                skr_version = '',
                updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                user_modified_skr03 = True,
                user_modified_skr04 = True
            )
        else:
            return PostingCategoryUpdate(
        )
        """

    def testPostingCategoryUpdate(self):
        """Test PostingCategoryUpdate"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
