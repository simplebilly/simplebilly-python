# coding: utf-8

"""
    SimpleBilly API

    Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

    The version of the OpenAPI document: 0.1.0
    Contact: support@simplebilly.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from simplebilly_api.models.pay_gap_report import PayGapReport

class TestPayGapReport(unittest.TestCase):
    """PayGapReport unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PayGapReport:
        """Test PayGapReport
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PayGapReport`
        """
        model = PayGapReport()
        if include_optional:
            return PayGapReport(
                by_job_title = [
                    simplebilly_api.models.job_title_gap.JobTitleGap(
                        employee_count = 0, 
                        female_mean_hourly = '', 
                        job_title = '', 
                        male_mean_hourly = '', 
                        mean_gap_pct = 1.337, 
                        median_gap_pct = 1.337, )
                    ],
                diverse_count = 0,
                employee_count = 0,
                female_count = 0,
                male_count = 0,
                mean_gap_pct = 1.337,
                median_gap_pct = 1.337,
                quartiles = [
                    simplebilly_api.models.quartile_band.QuartileBand(
                        female_share_pct = 1.337, 
                        hourly_median = '', 
                        male_share_pct = 1.337, 
                        quartile = '', )
                    ]
            )
        else:
            return PayGapReport(
                by_job_title = [
                    simplebilly_api.models.job_title_gap.JobTitleGap(
                        employee_count = 0, 
                        female_mean_hourly = '', 
                        job_title = '', 
                        male_mean_hourly = '', 
                        mean_gap_pct = 1.337, 
                        median_gap_pct = 1.337, )
                    ],
                diverse_count = 0,
                employee_count = 0,
                female_count = 0,
                male_count = 0,
                mean_gap_pct = 1.337,
                median_gap_pct = 1.337,
                quartiles = [
                    simplebilly_api.models.quartile_band.QuartileBand(
                        female_share_pct = 1.337, 
                        hourly_median = '', 
                        male_share_pct = 1.337, 
                        quartile = '', )
                    ],
        )
        """

    def testPayGapReport(self):
        """Test PayGapReport"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
