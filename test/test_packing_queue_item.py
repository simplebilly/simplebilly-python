# coding: utf-8

"""
    SimpleBilly API

    Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

    The version of the OpenAPI document: 0.1.0
    Contact: support@simplebilly.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from simplebilly_api.models.packing_queue_item import PackingQueueItem

class TestPackingQueueItem(unittest.TestCase):
    """PackingQueueItem unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PackingQueueItem:
        """Test PackingQueueItem
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PackingQueueItem`
        """
        model = PackingQueueItem()
        if include_optional:
            return PackingQueueItem(
                created_at = '',
                customer_id = '',
                delivery_note_printed = True,
                items = None,
                items_count = 56,
                label_printed = True,
                order_number = '',
                order_status = '',
                shipment_id = '',
                shipping_address = None,
                shipping_method = '',
                tracking_number = '',
                video_recording = ''
            )
        else:
            return PackingQueueItem(
                created_at = '',
                customer_id = '',
                delivery_note_printed = True,
                items = None,
                items_count = 56,
                label_printed = True,
                order_number = '',
                order_status = '',
                shipping_method = '',
        )
        """

    def testPackingQueueItem(self):
        """Test PackingQueueItem"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
