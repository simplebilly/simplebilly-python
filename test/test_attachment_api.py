# coding: utf-8

"""
    SimpleBilly API

    Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

    The version of the OpenAPI document: 0.1.0
    Contact: support@simplebilly.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from simplebilly_api.api.attachment_api import AttachmentApi


class TestAttachmentApi(unittest.TestCase):
    """AttachmentApi unit test stubs"""

    def setUp(self) -> None:
        self.api = AttachmentApi()

    def tearDown(self) -> None:
        pass

    def test_attachment_restore(self) -> None:
        """Test case for attachment_restore

        """
        pass

    def test_create_attachment(self) -> None:
        """Test case for create_attachment

        """
        pass

    def test_delete_attachment(self) -> None:
        """Test case for delete_attachment

        """
        pass

    def test_get_attachment(self) -> None:
        """Test case for get_attachment

        """
        pass

    def test_list_attachments(self) -> None:
        """Test case for list_attachments

        """
        pass

    def test_save_attachment_ocr_text(self) -> None:
        """Test case for save_attachment_ocr_text

        Persist client-side OCR output for an attachment.
        """
        pass


if __name__ == '__main__':
    unittest.main()
