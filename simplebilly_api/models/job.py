...
class Job(BaseModel):
    ...
    job_type: StrictStr = Field(description="Discriminator the worker dispatches on (e.g. \"webhook.deliver\").", alias="jobType")
    max_attempts: StrictInt = Field(alias="maxAttempts")
    payload: Optional[Any] = None
    run_at: Optional[datetime] = Field(default=None, description="Earliest execution time; None = run now.", alias="runAt")
    status: JobStatus = Field(description="pending | running | done | failed")
    __properties: ClassVar[List[str]] = ["attempts", "jobType", "maxAttempts", "payload", "runAt", "status"]
