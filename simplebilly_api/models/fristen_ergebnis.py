...
class FristenErgebnis(BaseModel):
    """
    FristenErgebnis
    """ # noqa: E501
    anzahl: Annotated[int, Field(strict=True, ge=0)]
    fristen: List[FristEintrag]
    __properties: ClassVar[List[str]] = ["anzahl", "fristen"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in fristen (list)
        _items = []
        if self.fristen:
            for _item_fristen in self.fristen:
                _items.append(_item_fristen.to_dict() if _item_fristen is not None else None)
            _dict['fristen'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "anzahl": obj.get("anzahl"),
            "fristen": [FristEintrag.from_dict(_item) for _item in obj["fristen"]] if obj.get("fristen") is not None else None
        })
        return _obj
